...
def getDegreeListsVertices_weighted(g,vertices,calcUntilLayer):
    degreeList = {}

    for v in vertices:
        degreeList[v] = getDegreeLists_weighted(g,v,calcUntilLayer)
...
def custom_sort_key(inner_list):
    # Create a tuple of elements in reverse order
    return tuple(reversed(inner_list))

def getDegreeLists_weighted(g, root, calcUntilLayer):
    t0 = time()
    ...
    while queue:
        ...
        l.append(g[vertex]['degree_hist'])

        for v in list(g[vertex].keys())[:-1]:
            if(vetor_marcacao[v] == 0):
                vetor_marcacao[v] = 1
                queue.append(v)
                pendingDepthIncrease += 1    
        ...
        if(timeToDepthIncrease == 0):
            lp = np.array(sorted(l, key=custom_sort_key))
            # lp = sorted(l, key=custom_sort_key)
            listas[depth] = lp
            l = deque()
...
def splitDegreeList_sdw(part,c,G,compactDegree):
    if(compactDegree):
        logging.info("Recovering compactDegreeList from disk...")
        degreeList = restoreVariableFromDisk('compactDegreeList')
    else:
...
def calc_distances_sdw(part, untilLayer,bin_coeffient, compactDegree = False):
    # scale_free = False
    # scale_free = True
    vertices = restoreVariableFromDisk('split-vertices-'+str(part))
    degreeList = restoreVariableFromDisk('split-degreeList-'+str(part))
    ...
    cont = 0
    dtw_distance_layer1 = {}
    def cost_weighted_bjsd(a_,b_):
        a = cost_hyper_base_dtw1_in_a[int(a_)]
        b = cost_hyper_base_dtw1_in_b[int(b_)]
        if (str((a,b)) not in dtw_distance_layer1.keys()):
            # dtw_distance_layer1[str((a,b))] = (np.e*(np.exp(distance.jensenshannon(np.array(a) * bin_coeffient, np.array(b) * bin_coeffient) + abs((max(a)-max(b))/(max(a)+max(b)))) - 1))
            dtw_distance_layer1[str((a,b))] = (np.e*(np.exp(distance.jensenshannon(np.array(a), np.array(b)) + abs((max(a)-max(b))/(max(a)+max(b)))) - 1))
        return dtw_distance_layer1[str((a,b))]
    if compactDegree: # TODO
        dist_func = cost_complex_vector_Euclidean_logscale_max 
        for v1,nbs in vertices.items():
            lists_v1 = degreeList[v1]
            lists_v1_new = {}
...
def calc_distances_all_weighted_presearch(vertices,list_vertices,degreeList,part,untilLayer, bin_coeffient, compactDegree = False):
    distances = {}
    cont = 0
    dtw_distance_layer1 = {}
    def cost_weighted_bjsd_pre0(a,b):
        # print (a,b)
        if (str((a,b)) not in dtw_distance_layer1.keys()):
            dtw_distance_layer1[str((a,b))] = (np.exp(distance.jensenshannon(np.array(a), np.array(b)) + abs((max(a)-max(b))/(max(a)+max(b))))  - 1)
        return dtw_distance_layer1[str((a,b))]
    ...
    if compactDegree: # TODO
        ...
        for v1 in vertices:
            ...
            for v2 in list_vertices[cont]:
                lists_v2 = degreeList[v2]
                dist = cost_weighted_bjsd_pre0(lists_v1[0][0], lists_v2[0][0])
                distances[v1,v2] = dist
            cont += 1
    # preprocess_consolides_distances(distances)
    saveVariableOnDisk(distances,'presearch-distances-'+str(part))
    return

def calc_distances_all_weighted_presearch_find_best(parts):
    all_distance = {}
    for part in range(1, 1+parts):
        distances = restoreVariableFromDisk('presearch-distances-'+str(part))
        all_distance.update(distances)
    all_weight = np.sort(np.array(list(all_distance.values())))
    
    length_weight = len(all_weight)
    distance_2point = []
    for i in range(-10,11):
        org_ = all_weight*np.e**i
        org_ = np.e**(-org_)
        org_ = (org_ - org_.min())/(org_.max()-org_.min())
        distance_2point.append(abs(org_[int(0.2*length_weight)]-0.2))
        
    logging.info(f"calc_distances_all_weighted_presearch_find_best best k={np.nanargmin(distance_2point)-10} from {distance_2point}")
    print (f"XXXXXXXXXXXXXXXX  {np.nanargmin(distance_2point)-10} from {distance_2point}")
    return np.nanargmin(distance_2point)-10
    
def calc_distances_all_weighted(vertices,list_vertices,degreeList,part,untilLayer, bin_coeffient, compactDegree = False, best_k = 1):
    distances = {}
    cont = 0
    dtw_distance_layer1 = {}
    def cost_weighted_bjsd(a_,b_):
        ...
        if (str((a,b)) not in dtw_distance_layer1.keys()):
            # dtw_distance_layer1[str((a,b))] = np.e*(np.exp(distance.jensenshannon(np.array(a) * bin_coeffient, np.array(b) * bin_coeffient) + abs((max(a)-max(b))/(max(a)+max(b))))  - 1)
            # dtw_distance_layer1[str((a,b))] = (np.e**(best_k+3))*(np.exp(distance.jensenshannon(np.array(a), np.array(b)) + abs((max(a)-max(b))/(max(a)+max(b))))  - 1)
            dtw_distance_layer1[str((a,b))] = (np.e**(best_k))*(np.exp(distance.jensenshannon(np.array(a), np.array(b)) + abs((max(a)-max(b))/(max(a)+max(b))))  - 1)
        return dtw_distance_layer1[str((a,b))]
...
def exec_bfs_weighted(G,workers,calcUntilLayer):
    futures = {}
    degreeList = {}

    t0 = time()
    vertices = list(reversed(sorted(list(G.keys()))))
    parts = workers
    chunks = partition(vertices,parts)
    for c in chunks:
        dl = getDegreeListsVertices_weighted(G,c,calcUntilLayer)
        degreeList.update(dl)
    with ProcessPoolExecutor(max_workers=workers) as executor:

        part = 1
        for c in chunks:
            job = executor.submit(getDegreeListsVertices_weighted,G,c,calcUntilLayer)
            futures[job] = part
            part += 1
...
def generate_distances_network(workers):
    ...
    os.system("rm "+returnPathWeightedS2V()+"weights_distances-layer-*.pickle")
    ...
    t0 = time()
    os.system("rm "+returnPathWeightedS2V()+"graphs-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
    ...
    t0 = time()
    os.system("rm "+returnPathWeightedS2V()+"distances_nets_weights-layer-*.pickle")
    os.system("rm "+returnPathWeightedS2V()+"alias_method_j-layer-*.pickle")
    os.system("rm "+returnPathWeightedS2V()+"alias_method_q-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
